...
export async function POST(request: Request) {
    try {
        ...
        if (!profile || profile.role !== 'centro') {
            return NextResponse.json({ error: "Solo los usuarios de tipo Centro pueden inscribir alumnos" }, { status: 403 });
        }

        const body = await request.json();
        const { email, password, full_name, course_group, student_code, department } = body;
        ...
        return NextResponse.json({ success: true, student: data.user });
    } catch (err: any) {
        console.error("Error en API de inscripción de alumno:", err);
